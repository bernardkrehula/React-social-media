...
const SinglePost = ({ post, deletePost, user, editPost, editComment, addNewComment, likePost, unlikePost, deleteComment}) => {
    ...
    const saveEditPostChanges = () => {
        editPost(id, postInput); 
        enablePostEditing();
    };
    const enablePostEditing = () => setIsEdited(prev => !prev);
    ...
    return(
        <>
            <div className='post'>
                ...
                {displayOptions ? <DotOptions postId={id} enablePostEditing={enablePostEditing} displayOptions={displayOptions} optionsDisplayed={optionsDisplayed} saveEditPostChanges={saveEditPostChanges} deletePost={deletePost} /> : ''}
                {isEdited ? <textarea value={postInput} onChange={editPostOnChange}/> : <p>{postInput}</p>}
