...
const Comment = ({postId, comment, deleteComment, editComment, setNewComment }) => {
    const [ isCommentDisabled, setCommentDisabled ] = useState(true);
    const [ displayOptions, setDisplayOptions ] = useState(false);
    const { id, content, userName, userLastName, userImg, isCommentByUser, isCommentEdited } = comment;
    ...
    const manageEditComment = () => {
        editComment(postId, id, commentInput);
    }
    ...
    return(
        <div className='comment'>
            ...
            <div className='comment-info'>
                <h2>{userName} {userLastName} {isCommentEdited ? '(edited)' : ''}</h2>
            ...
            {displayOptions ? <DotOptions postId={postId} commentId={id} manageEditComment={manageEditComment} enableCommentEditing={enableCommentEditing} isCommentByUser={isCommentByUser} variation='commentOptions' setCommentDisabled={setCommentDisabled} activateOptions={activateOptions} isCommentDisabled={isCommentDisabled} deleteComment={deleteComment}/> : ''}
        </div>
    )
}
